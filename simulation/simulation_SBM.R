# simulation_SBM.R
# Simulation for Partial Network Recovery on Stochastic Block Model
...
simulation_SBM <- function(N, K, P, T, par, q) {
  # Generate group assignments and adjacency matrix
  b <- block_N(N, K)
  A <- get_A(N, b, P)
...
write.table(result1,
  file = paste0("result_SBM_N=", N, "_T=", T, ".txt"),
