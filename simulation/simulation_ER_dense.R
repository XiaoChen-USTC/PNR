...
# Simulation for Partial Network Recovery on dense Erdős-Rényi random graphs
...
simulation_ER_dense <- function(N, T, par) {
  # Generate adjacency matrix A using dense ER model
  A <- get_A_ER_d(N)
  ...
  # Oracle method 1: using true parameters
  for (i in 1:(N - m)) {
    Y_o1 <- Y[(m + 1):N, ] - par[2] * t(A12) %*% (Y[1:m, ]) - par[1]
    X_o1 <- t(par[2] * (Y[, ]))
    X_o1 <- X_o1[, -c(1:m, i + m)]
  ...
  for (k in 1:1) {
    ...
    for (i in 1:(N - m)) {
      # Oracle 2: using true parameters
      Y_o2 <- Y[(m + 1):N, ] - par[2] * t(A12) %*% (Y[1:m, ]) - par[1]
      X_o2 <- t(par[2] * (Y[, ]))
      X_o2 <- X_o2[, -c(1:m, i + m)]
      SL_o2 <- SL(X_o2, Y_o2, i, N, m, A)

      # Estimation: using estimated parameters
      Y_e <- Y[(m + 1):N, ] - rhoEst[2] * t(A12) %*% (Y[1:m, ]) - rhoEst[1]
      X_e <- t(rhoEst[2] * (Y[, ]))
      X_e <- X_e[, -c(1:m, i + m)]
      SL_e <- SL(X_e, Y_e, i, N, m, A)

      # Lasso: using true parameters
      Y_l <- Y[(m + 1):N, ] - par[2] * t(A12) %*% (Y[1:m, ]) - par[1]
      X_l <- t(par[2] * (Y[, ]))
      X_l <- X_l[, -c(1:m, i + m)]
...
write.table(result1,
  file = paste0("result_ER_dense_N=", N, "_T=", T, ".txt"),
